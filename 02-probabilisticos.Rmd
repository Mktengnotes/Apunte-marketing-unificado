...
$$\begin{aligned}
L(\lambda) &= \prod_{i \in A} f(t_i \mid \lambda) \prod_{i \notin A} (1 - P(T \leq t)) \\
           &= \prod_{i \in A} (\lambda e^{-\lambda t_i}) \prod_{i \notin A} (e^{-\lambda \tau_i})
\end{aligned} \tag{2.18}$$
...
Usando el modelo individual en (2.34) y la distribución en (2.38), se
...
Using the individual model in (2.34) and the distribution in (2.38),
...
-   **Esperanza de la distribución Gamma**
    ...
    $$g(\lambda|r,\alpha) = \frac{\alpha^r}{\Gamma(r)} \lambda^{r-1} e^{-\alpha\lambda} \tag{2.64}$$
    La esperanza de una variable aleatoria $\lambda$ que sigue esta
    distribución es: $$
    \mathbb{E}[\lambda] = \frac{r}{\alpha} \tag{2.65}$$Este resultado se obtiene a partir de la definición de valor
...
-   **Esperanza de la distribución Beta**
    ...
    $$g(\theta|\alpha,\beta) = \frac{\theta^{\alpha-1}(1-\theta)^{\beta-1}}{B(\alpha,\beta)} \tag{2.66}$$
    Por definición de valor esperado para una variable continua:
    $$\mathbb{E}[\theta] = \int_{0}^{1} \theta \cdot g(\theta|\alpha,\beta) d\theta \tag{2.67}$$
...
-   **Expectation of the Gamma distribution**
    ...
    distribution is:
    $$\mathbb{E}[\lambda] = \frac{r}{\alpha} \tag{2.65}$$
...
-   **Expectation of the Beta distribution**
    ...
    probability density function is:
    $$g(\theta|\alpha,\beta) = \frac{\theta^{\alpha-1}(1-\theta)^{\beta-1}}{B(\alpha,\beta)} \tag{2.66}$$
    By definition of expected value for a continuous variable:
    $$\mathbb{E}[\theta] = \int_{0}^{1} \theta \cdot g(\theta|\alpha,\beta) d\theta \tag{2.67}$$
    Which simplifies to:
    $$\mathbb{E}[\theta] = \frac{\alpha}{\alpha+\beta} \tag{2.68}$$
...
-   **Función de Verosimilitud**: El comportamiento de abandono
    ...
    período $t$: $$ \mathbb{P}(T=t|\theta) = \theta(1-\theta)^{t-1} \tag{2.70}$$
...
-   **Función de Verosimilitud**: El comportamiento del tiempo hasta el
    ...
    instante $t$:
    $$ \mathbb{P}(T=t|\lambda) = f(t|\lambda) = \lambda e^{-\lambda t} \tag{2.76}$$
...
(2.40). Una pregunta válida que se puede hacer es cuál es la tasa de
...
In the choice model where $P(X_s = x_s)$ was defined in (2.40), a valid
...
2.  Suponga ahora que, en vez de describir el número de eventos de cada
    ...
    Para las descripciones anteriores considere que $n_{jkt}$ es el
    número total de beneficiarios que tuvieron algún gasto de salud en
    el trimestre t dentro del segmento j de la empresa k y que
    $m_{jkt}$ es el número total de beneficiarios dentro de ese
    segmento, empresa y trimestre.
...
2.  Suppose now that, instead of describing the number of events for
    ...
    beneficiaries who had at least one health expenditure in quarter
    $t$ within segment $j$ of company $k$, and let $m_{jkt}$ be the
    total number of beneficiaries within that segment, company, and
    quarter.
...
a.  Discuta si el programa *TuBienestar* ayuda a describir el
    comportamiento de los beneficiarios y si el programa *TuBienestar*
    es bueno para la salud de los beneficiarios.
b.  En otros mercados el valor esperado de la probabilidad que un
    beneficiario exhiba un evento de salud en un trimestre dado es
    0.16. ¿Cómo testearía si los datos de Chile son distintos a ese
    valor medio internacional?
...
a.  Discuss whether the *TuBienestar* program helps describe
    beneficiaries' behavior and whether the *TuBienestar* program is
    good for beneficiaries' health.
b.  In other markets, the expected value of the probability that a
    beneficiary exhibits a health event in a given quarter is 0.16. How
    would you test whether the Chilean data differ from that
...
3.  Suppose three models are now estimated to describe the duration of
    ...
    The maximum-likelihood estimators are shown below.
    ...
    Choose two of the models and, for each one, calculate the
    probability that a discrepancy for a cleaning product, whose price
    difference is 201, lasts more than 3 days (leave the symbolic
    expression; a numerical value is not required).
...
### Problema 10: Retraso en graduaciones universitarias {.unnumbered}

::: {.lang-es}

Uno de los fenómenos que ha acompañado el desarrollo económico de Chile
en las últimas décadas es el aumento explosivo de la matrícula
universitaria. Aunque un mayor nivel de educación tiene impactos
universalmente favorables para el bienestar de las naciones, la
provisión de los servicios educacionales es costosa desde un punto de
vista privado y público. Esto motiva a que las distintas casas de
estudio pongan especial atención a la duración de los estudios
universitarios. Un mejor entendimiento de este fenómeno permitiría
orientar decisiones de gestión de recursos y poner atención al proceso
de prestación, con foco a tener un mejor balance entre los tiempos
nominales y efectivos.

De acuerdo con las estadísticas disponibles, los retrasos pueden
constituir un problema serio en el sistema universitario nacional. En
la Tabla \@ref(tab:prob-p10-duracion) se despliegan las duraciones
promedio de los titulados del sistema entre los años 2012 y 2021, las
que oscilan en torno a los 10 semestres. Esta cifra contrasta con la
duración formal de los planes de estudios que, al 2021, ascendía a los
7.6 semestres en promedio.

:::

::: {.lang-en}

One of the phenomena that has accompanied Chile's economic development
in recent decades is the explosive increase in university enrollment.
Although a higher level of education has universally favorable impacts
on the well-being of nations, the provision of educational services is
costly from both a private and public standpoint. This motivates higher
education institutions to pay special attention to the duration of
university studies. A better understanding of this phenomenon would
inform resource management decisions and the design of academic
programs, with the aim of achieving a better balance between nominal
and effective durations.

According to available statistics, delays can constitute a serious
problem in the national university system. Table
\@ref(tab:prob-p10-duracion) shows the average graduation durations in
the system between 2012 and 2021, oscillating around 10 semesters.
This figure contrasts with the formal duration of study plans, which
in 2021 averaged 7.6 semesters.

:::

::: {.lang-es}

Table: (\#tab:prob-p10-duracion) Duración efectiva de las carreras de la educación superior en Chile.
...
Table: (\#tab:prob-p10-duracion) Effective duration of higher education programs in Chile.

|           | 2012 | 2013 | 2014 | 2015 | 2016 | 2017 | 2018 | 2019 | 2020 | 2021 |
|:----------|:----:|:----:|:----:|:----:|:----:|:----:|:----:|:----:|:----:|:----:|
| Semesters | 10.1 | 10.1 | 10.0 |  9.7 |  9.6 |  9.6 |  9.8 |  9.7 |  9.6 | 10.1 |

:::

::: {.lang-es}

Al descomponer por programas, los que más demoran en graduar a sus
estudiantes son las carreras profesionales con licenciatura impartidas
en universidades, con un promedio de 13,3 semestres. Considerando que
la duración formal promedio de estos programas es de 10,1 semestres,
se registra un retraso promedio de 3,2 semestres. En particular, las
carreras de ingeniería están entre las que tienen mayor duración real.
Por ejemplo, Ingeniería Civil Electrónica tiene una duración promedio
de 16,8 semestres, Ingeniería Civil Metalúrgica una duración de 16,7
semestres e Ingeniería Civil Eléctrica dura 16,5 semestres en
promedio.

Para entender en profundidad qué factores son los que más influyen en
el retraso en las graduaciones, un equipo de la escuela de Ingeniería
de la Universidad de Chile ha armado una base de datos con los
graduados de las últimas cohortes, en que se registran las siguientes
variables:
...
-   *Region*: 1 si el colegio de egreso no está ubicado en la Región
    Metropolitana.
...
-   *NSem*: número de semestres que demoró el o la estudiante en
    graduarse.

El análisis se centra en el comportamiento del número de semestres de
retraso ($NRet$), el que, por simplicidad, puede suponerse que se
calcula como $NRet = NSem - 12$. A partir de una exploración inicial
de datos, se ha considerado que esta variable $NRet$ puede aproximarse
a través de una distribución de Poisson.

1.  Suponga que observa dos individuos, con tiempos de graduación de
    12 ($NRet = 0$) y 13 semestres ($NRet = 1$). ¿Qué valor de
    $\lambda$ le parece más verosímil: $\lambda = 1$ o $\lambda = 2$?

    *Hint:* si lo necesita, puede usar los valores numéricos de la
    Tabla \@ref(tab:prob-p10-logs).

:::

::: {.lang-en}

When broken down by program, those that take the longest to graduate
their students are professional programs with a *licenciatura* offered
by universities, averaging 13.3 semesters. Given that the formal
average duration of these programs is 10.1 semesters, the average
delay is 3.2 semesters. Engineering programs are among those with the
longest effective duration. For example, Electronic Engineering takes
an average of 16.8 semesters, Metallurgical Engineering 16.7
semesters, and Electrical Engineering 16.5 semesters.

To better understand which factors most influence graduation delays,
a team from the School of Engineering at Universidad de Chile has
assembled a database of graduates from recent cohorts, recording the
following variables:

-   *AñoIng*: year of admission to the university.
-   *EdadIng*: age at admission.
-   *NTrabaja*: number of semesters worked while studying.
-   *Beca*: 1 if the student held an academic merit scholarship.
-   *Nfam*: number of household members.
-   *Especialidad*: program from which the student graduated.
-   *Particular*: 1 if the student graduated from a private fee-paying
    school.
-   *NEM*: high school grade-point average.
-   *Region*: 1 if the high school is located outside the Metropolitan
    Region.
-   *PMAT*: admission test score in mathematics.
-   *PCIE*: admission test score in sciences.
-   *PLEN*: admission test score in language.
-   *NSem*: number of semesters the student took to graduate.

The analysis focuses on the number of semesters of delay ($NRet$),
which, for simplicity, may be assumed to satisfy $NRet = NSem - 12$.
An initial exploratory analysis suggests that $NRet$ can be
approximated by a Poisson distribution.

1.  Suppose two individuals are observed with graduation times of 12
    semesters ($NRet = 0$) and 13 semesters ($NRet = 1$). Which value
    of $\lambda$ seems more likely: $\lambda = 1$ or $\lambda = 2$?

    *Hint:* if needed, the numerical values in Table
    \@ref(tab:prob-p10-logs) may be useful.

:::

::: {.lang-es}

Table: (\#tab:prob-p10-logs) Valores numéricos de $\log(x)$ para una selección de valores de $x$.
...
Table: (\#tab:prob-p10-logs) Numerical values of $\log(x)$ for a selection of $x$.
...
2.  Suponga que se quiere estimar un modelo de Poisson con dos clases
    latentes, con parámetros $\lambda_{\text{high}}$ y
    $\lambda_{\text{low}}$ ($\lambda_{\text{low}} < \lambda_{\text{high}}$).
    Escriba una expresión para la probabilidad de que un estudiante
    se atrase, condicional en que pertenece al segmento con
    $\lambda_{\text{high}}$.

3.  Suponga que ahora se quiere describir los meses de retraso como un
    modelo de conteo que considere tanto heterogeneidad observable
    como no observable.

    a.  Para este modelo, escriba una expresión para la tasa de
        retraso considerando al menos 2 variables observables y
        explique por qué la inclusión de esas variables tiene
        sentido. Determine cuál es el signo esperado para el
        coeficiente correspondiente.
    b.  Usando la definición de la tasa de retraso de la pregunta
        anterior, agregue heterogeneidad no observable y escriba la
        log-verosimilitud de observar todo el conjunto de datos.

4.  Después de terminar el análisis y presentar los resultados al
    equipo de trabajo, un estudiante en práctica levanta la mano y le
    manifiesta que todo el análisis presentado es incorrecto. Esto
    porque el problema corresponde a un modelo de duración en tiempo
    discreto y, por tanto, en vez de usar un modelo de Poisson
    debiera usarse un modelo geométrico desplazado. Discuta respecto
    a la veracidad del argumento del estudiante.

:::

::: {.lang-en}

2.  Suppose a Poisson model with two latent classes is to be
    estimated, with parameters $\lambda_{\text{high}}$ and
    $\lambda_{\text{low}}$ ($\lambda_{\text{low}} < \lambda_{\text{high}}$).
    Write an expression for the probability that a student is
    delayed, conditional on belonging to the segment with
    $\lambda_{\text{high}}$.

3.  Suppose now that one wants to describe the months of delay as a
    count model that incorporates both observable and unobservable
    heterogeneity.

    a.  For this model, write an expression for the delay rate
        considering at least two observable variables and explain
        why their inclusion is sensible. Determine the expected
        sign of the corresponding coefficient.
    b.  Using the delay rate from the previous part, add unobservable
        heterogeneity and write the log-likelihood of observing the
        entire dataset.

4.  After completing the analysis and presenting the results to the
    team, an intern raises their hand and argues that the entire
    analysis is incorrect, because the problem corresponds to a
    discrete-time duration model and therefore, instead of a Poisson
    model, a shifted geometric model should be used. Discuss the
    validity of the intern's argument.
...
1.  Podríamos aplicar directamente la definición de verosimilitud,
    pero numéricamente es más cómodo trabajar con la
    log-verosimilitud:

    $$
    LL(\lambda) = \ln\!\big(\mathbb{P}(NRet_1 = 0 \mid \lambda)\big) + \ln\!\big(\mathbb{P}(NRet_2 = 1 \mid \lambda)\big) = -\lambda + \ln\!\big(\lambda e^{-\lambda}\big).
    $$
    ...
    $$
    \begin{aligned}
    LL(\lambda = 1) &= -1 + \ln\!\big(1 \cdot e^{-1}\big) = -2, \\
    LL(\lambda = 2) &= -2 + \ln(2) + \ln\!\big(e^{-2}\big) = -2 + 0.693 - 2 = -3.307.
    \end{aligned}
    $$

    Con lo que verificamos que $\lambda = 1$ es más verosímil que
    $\lambda = 2$.
...
1.  We could apply the definition of likelihood directly, but it is
    numerically more convenient to work with the log-likelihood:
    ...
    Hence $\lambda = 1$ is more likely than $\lambda = 2$.

:::

::: {.lang-es}

2.  Condicional en la pertenencia al segmento con
    $\lambda_{\text{high}}$, los retrasos siguen un proceso de Poisson
    evaluado en $\lambda = \lambda_{\text{high}}$. Luego,

    $$
    \mathbb{P}\!\big(NRet > 0 \mid \lambda = \lambda_{\text{high}}\big) = 1 - \mathbb{P}\!\big(NRet = 0 \mid \lambda = \lambda_{\text{high}}\big) = 1 - \exp\!\big(-\lambda_{\text{high}}\big).
    $$
...
2.  Conditional on membership in the segment with
    $\lambda_{\text{high}}$, delays follow a Poisson process with
    rate $\lambda = \lambda_{\text{high}}$. Therefore,
...
3.  La primera parte trata de heterogeneidad observable; la segunda
    agrega heterogeneidad no observable.

    a.  La especificación es la misma vista en clases:

        $$
        \hat{\lambda}_i = \lambda_0 \exp\!\big(\beta' x_i\big),
        $$

        de modo que basta listar y justificar las componentes de
        $x_i$. Algunos ejemplos:

        -   *PMAT*: estudiantes con mejores notas en pruebas de
            matemáticas suelen tener mejores resultados y se espera
            que se retrasen menos (coeficiente negativo).
        -   *NEM*: estudiantes con mejores notas en enseñanza media
            se retrasan menos (coeficiente negativo).
        -   *Particular*: estudiantes de colegios particulares
            tienden a tener mejores resultados, se espera que se
            retrasen menos (coeficiente negativo).
        -   *Beca*: estudiantes con beca tienen mejores notas e
            incentivos para no retrasarse, por lo que se espera un
            coeficiente negativo.

    b.  Al agregar heterogeneidad no observable se obtiene un modelo
        de regresión binomial negativa. La log-verosimilitud queda

        $$
        LL\!\big(\hat{\lambda}_i\big) = \sum_i \ln\!\Bigg( \frac{\Gamma(r + n_i)}{\Gamma(r)\, n_i!} \left(\frac{\alpha}{\alpha + \exp(\beta' x_i)}\right)^{r} \left(\frac{\exp(\beta' x_i)}{\alpha + \exp(\beta' x_i)}\right)^{n_i} \Bigg).
        $$
...
3.  The first part deals with observable heterogeneity; the second
    adds unobservable heterogeneity.

    a.  The specification is the same one seen in class:

        $$
        \hat{\lambda}_i = \lambda_0 \exp\!\big(\beta' x_i\big),
        $$

        so it suffices to list and justify the components of $x_i$.
        Some examples:

        -   *PMAT*: students with better mathematics scores tend to
            perform better and are expected to be less delayed
            (negative coefficient).
        -   *NEM*: students with higher high school GPAs are less
            delayed (negative coefficient).
        -   *Particular*: students from private schools tend to
            perform better and are expected to be less delayed
            (negative coefficient).
        -   *Beca*: scholarship holders have better grades and
            stronger incentives to avoid delays, so a negative
            coefficient is expected.

    b.  Adding unobservable heterogeneity yields a negative binomial
        regression. The log-likelihood is
...
4.  El modelo geométrico desplazado es una alternativa razonable para
    caracterizar los retrasos. Sin embargo, un modelo de conteo
    también es válido: la elección entre uno y otro depende de cómo
    se interprete $NRet$ (cuántos semestres adicionales son
    necesarios para graduarse, en una óptica de duración, frente al
    número de eventos de atraso, en una óptica de conteo). Para
    determinar cuál resulta más adecuado en este caso conviene
    revisar métricas de ajuste y de pronóstico fuera de muestra.

:::

::: {.lang-en}

4.  The shifted geometric model is a reasonable alternative for
    characterizing delays. However, a count model is also valid: the
    choice between the two depends on how $NRet$ is interpreted (how
    many additional semesters are needed to graduate, from a duration
    perspective, versus the number of delay events, from a counting
    perspective). To decide which one fits the data better, one
    should compare in-sample fit and out-of-sample predictive
    metrics.
...
### Problema 11: Tasas de respuesta en encuestas de educación superior {.unnumbered}

::: {.lang-es}

Los paneles de encuestas siguen siendo una herramienta importante para
recoger la valoración que tienen los usuarios sobre sus experiencias en
distintas industrias de servicios. Uno de los desafíos actuales de la
administración de estos paneles es las bajas tasas de respuesta, lo que
dificulta la recopilación de datos y potencialmente introduce sesgos en
las percepciones medidas. A continuación, propondremos el estudio de
las tasas de respuesta en el contexto de las evaluaciones que proveen
los estudiantes a las encuestas semestrales en cada una de las materias
que cursan.

Para estudiar el fenómeno, se ha preparado un panel de encuestados
caracterizados por el número de matrícula, la carrera que estudian y el
año de ingreso, y una muestra de 94 encuestas realizadas entre 2023 y
2025 en una institución de educación superior. Para cada encuesta,
conocemos la fecha de envío, el número de días en que la encuesta está
abierta y el tipo de encuesta. La variable central de análisis es una
variable binaria que indica si el estudiante responde o no a la
encuesta. La Tabla \@ref(tab:prob-p11-panel) despliega una muestra con
los primeros 6 registros del panel. Notar que este panel solo registra
los casos de estudiantes que recibieron una encuesta; si un alumno no
recibe una encuesta, esa combinación de estudiante-encuesta no se
registra en el panel.

:::

::: {.lang-en}

Survey panels remain an important tool for collecting users'
assessments of their experiences across various service industries. One
of the current challenges in managing these panels is the low response
rate, which hampers data collection and potentially introduces biases
in the measured perceptions. Below, we study response rates in the
context of evaluations that students provide in semester surveys for
each of the courses they take.

To study the phenomenon, a panel of respondents has been prepared,
characterized by enrollment number, field of study, and year of
admission, along with a sample of 94 surveys conducted between 2023
and 2025 at a higher education institution. For each survey, we know
the date it was sent, the number of days it remained open, and the
type of survey. The central variable of analysis is a binary variable
indicating whether the student responds to the survey or not. Table
\@ref(tab:prob-p11-panel) displays a sample of the first 6 records
from the panel. Note that this panel only records cases where a
student received a survey; if a student does not receive a survey, that
student-survey combination is not recorded in the panel.

:::

::: {.lang-es}

Table: (\#tab:prob-p11-panel) Muestra de los primeros seis registros del panel de encuestados.
...
Table: (\#tab:prob-p11-panel) Sample of the first six records from the survey panel.

| NMat   | Field       | Year Adm. | IdEnc | Course Code | N | Date Sent   | Days | Survey Type | Response |
|:-------|:------------|:---------:|:-----:|:-----------:|:-:|:-----------:|:----:|:-----------:|:--------:|
| 945001 | Engineering |   2019    | 4501  |   ECO101    |   | 2023-11-15  |  14  |  Teaching   |   Yes    |
| 033012 | Psychology  |   2020    | 4502  |   PSI205    |   | 2023-11-15  |  14  |  Teaching   |    No    |
| 111005 | Law         |   2021    | 4503  |   DER110    |   | 2023-11-16  |   7  |  Welfare    |    No    |
| 945001 | Engineering |   2019    | 4504  |   MAT200    |   | 2023-11-15  |  21  |  Teaching   |    No    |
| 200554 | Medicine    |   2022    | 4505  |   MED010    |   | 2023-11-17  |  14  |  Teaching   |   Yes    |
| 033012 | Psychology  |   2020    | 4506  |   SOC100    |   | 2023-11-15  |  14  |  Welfare    |    No    |

:::

::: {.lang-es}

1.  Escriba la log-verosimilitud de un modelo de conteo con
    heterogeneidad observable que describa el número de respuestas
    provistas por cada estudiante. En su desarrollo, asuma que la tasa
    de respuesta anual se puede escribir como $\lambda_i(\mathbf{x}_i)$
    donde $\mathbf{x}_i$ es un vector que caracteriza al estudiante $i$.
    De necesitar cualquier otro dato, explique cómo calcularlo a partir
    de los datos disponibles en el panel.

2.  Escriba la log-verosimilitud de un modelo de conteo con
    heterogeneidad observable que describa el número de encuestas
    recibidas por cada estudiante. En su desarrollo, asuma que la tasa
    de recepción anual se puede escribir como $\mu_i(\mathbf{x}_i)$
    donde $\mathbf{x}_i$ es un vector que caracteriza al estudiante $i$.
    De necesitar cualquier otro dato, explique cómo calcularlo a partir
    de los datos disponibles en el panel.

    *Hint: para las dos preguntas anteriores, puede asumir que todos los
    estudiantes de la muestra están activos durante los tres años de
    observación.*

3.  Un estudiante plantea que una de las principales razones que
    explican las bajas tasas de respuesta es la gran cantidad de
    encuestas que envía la institución, que abruman al estudiantado. En
    particular, el estudiante plantea que la tasa de respuesta ha ido
    bajando porque la tasa de envío ha ido subiendo.

    a.  Para testear que la institución ha ido aumentando el número de
        encuestas, el estudiante plantea que debiera estimarse un modelo
        homogéneo independiente para cada año y luego comparar los
        valores de los estimadores máximos verosímiles
        $\hat{\lambda}_{2023}$, $\hat{\lambda}_{2024}$ y
        $\hat{\lambda}_{2025}$ usando un test de ratios de
        verosimilitud. Comente sobre la adecuación de la propuesta del
        estudiante.

    b.  Más allá de la opinión que tenga de la propuesta anterior del
        estudiante, ¿cómo haría usted para comprobar si efectivamente la
        institución ha mantenido o aumentado su tasa de envíos?

4.  Para avanzar en la comprensión del problema, se han estimado cuatro
    modelos de conteo, como se indica en la Tabla
    \@ref(tab:prob-p11-modelos). En la lista se incluyen tanto modelos
    homogéneos, como modelos que incorporan heterogeneidad observable y
    no observable.

    Table: (\#tab:prob-p11-modelos) Estimadores máximos verosímiles de cuatro modelos de conteo.

    | Variable     | Poisson Homogéneo | Poisson Heterogéneo | NBD Homogéneo | NBD Heterogéneo |
    |:-------------|:-----------------:|:-------------------:|:-------------:|:---------------:|
    | $\lambda_0$  | 12,58 \*\*\*      | 12,19 \*\*\*        |               |                 |
    | $\alpha$     |                   |                     | 0,45 \*\*     | 0,63 \*\*       |
    | $r$          |                   |                     | 3,52 \*\*\*   | 4,15 \*\*\*     |
    | $X_1$        |                   | 0,32 \*\*            |               | 0,42 \*\*\*     |
    | $X_2$        |                   | 0,08 .               |               | 0,15 \*         |
    ...
    Para el modelo que mejor ajuste a los datos, calcule la tasa esperada
    de un estudiante, para el próximo año, condicional a que ha contestado
    dos encuestas en el año recién pasado.

5.  Proponga y justifique 2 variables para incluir en el vector
    $\mathbf{x}_i$, distintas a la fecha de envío. Junto con explicar
    por qué piensa que hace sentido incluir esas variables, hipotetice
    la dirección esperada del efecto.

6.  Los modelos de conteo propuestos anteriormente suponen que las tasas
    de respuesta no dependen del número de encuestas realizadas. Esto
    es, dos estudiantes con las mismas características debieran generar,
    en promedio, las mismas respuestas, independientemente de si uno
    recibió más encuestas que el otro. Escriba la log-verosimilitud de
    un modelo que permita tener en cuenta que el número de respuestas
    generadas está asociado al número de encuestas recibidas.

:::

::: {.lang-en}

1.  Write the log-likelihood of a count model with observable
    heterogeneity that describes the number of responses provided by
    each student. In your derivation, assume that the annual response
    rate can be written as $\lambda_i(\mathbf{x}_i)$ where
    $\mathbf{x}_i$ is a vector characterizing student $i$. If you need
    any additional data, explain how to compute it from the data
    available in the panel.

2.  Write the log-likelihood of a count model with observable
    heterogeneity that describes the number of surveys received by each
    student. In your derivation, assume that the annual reception rate
    can be written as $\mu_i(\mathbf{x}_i)$ where $\mathbf{x}_i$ is a
    vector characterizing student $i$. If you need any additional data,
    explain how to compute it from the data available in the panel.

    *Hint: for the two questions above, you may assume that all students
    in the sample are active during the three years of observation.*

3.  A student argues that one of the main reasons explaining low
    response rates is the large number of surveys sent by the
    institution, which overwhelm students. Specifically, the student
    argues that the response rate has been declining because the sending
    rate has been rising.

    a.  To test whether the institution has been increasing the number
        of surveys, the student proposes estimating an independent
        homogeneous model for each year and then comparing the maximum
        likelihood estimates $\hat{\lambda}_{2023}$,
        $\hat{\lambda}_{2024}$, and $\hat{\lambda}_{2025}$ using a
        likelihood ratio test. Comment on the adequacy of the student's
        proposal.

    b.  Regardless of your opinion on the student's proposal above, how
        would you verify whether the institution has maintained or
        increased its sending rate?

4.  To advance the understanding of the problem, four count models have
    been estimated, as shown in Table \@ref(tab:prob-p11-modelos). The
    list includes both homogeneous models and models incorporating
    observable and unobservable heterogeneity.

    Table: (\#tab:prob-p11-modelos) Maximum likelihood estimates of four count models.

    | Variable     | Poisson Homog. | Poisson Heterog. | NBD Homog.  | NBD Heterog. |
    |:-------------|:--------------:|:----------------:|:-----------:|:------------:|
    | $\lambda_0$  | 12.58 \*\*\*   | 12.19 \*\*\*     |             |              |
    | $\alpha$     |                |                  | 0.45 \*\*   | 0.63 \*\*    |
    | $r$          |                |                  | 3.52 \*\*\* | 4.15 \*\*\*  |
    ...
    For the model that best fits the data, calculate the expected rate for
    a student, for the next year, conditional on having answered two surveys
    in the past year.

5.  Propose and justify 2 variables to include in the vector
    $\mathbf{x}_i$, other than the date the survey was sent. Along with
    explaining why you think it makes sense to include those variables,
    hypothesize the expected direction of the effect.

6.  The count models proposed above assume that response rates do not
    depend on the number of surveys administered. That is, two students
    with the same characteristics should generate, on average, the same
    responses, regardless of whether one received more surveys than the
    other. Write the log-likelihood of a model that accounts for the
    fact that the number of responses generated is associated with the
...
1.  Asumimos que el número de respuestas se distribuye Poisson con tasa
    $\lambda_i(\mathbf{x}_i)$. Como todos los estudiantes están activos
    durante los tres años de observación, la exposición es $T = 3$.
    Entonces la log-verosimilitud viene dada por:

    $$
    LL = \sum_i \ln\!\Big[\Pr(N_i = n_i)\Big]
       = \sum_i \ln\!\left[
         \frac{\big(3\,\lambda_i(\mathbf{x}_i)\big)^{n_i}\,
               e^{-3\,\lambda_i(\mathbf{x}_i)}}{n_i!}
         \right],
    $$

    donde $n_i$ es el número de respuestas del estudiante $i$, que se
    obtiene contando las filas del panel en que $\text{NMat} = i$ y
    $\text{Respuesta} = \text{Sí}$.

2.  La pregunta es conceptualmente idéntica a la anterior, pero se
    estima con otros datos. Asumiendo que el número de encuestas
    recibidas se distribuye Poisson con tasa $\mu_i(\mathbf{x}_i)$, la
    log-verosimilitud viene dada por:

    $$
    LL = \sum_i \ln\!\Big[\Pr(M_i = m_i)\Big]
       = \sum_i \ln\!\left[
         \frac{\big(3\,\mu_i(\mathbf{x}_i)\big)^{m_i}\,
               e^{-3\,\mu_i(\mathbf{x}_i)}}{m_i!}
         \right],
    $$

    donde $m_i$ es el número de encuestas recibidas por el
    estudiante $i$, que se obtiene contando las filas del panel en que
    $\text{NMat} = i$ (independiente de la respuesta).

3.  Las dos partes pueden contestarse separadamente:

    a.  La propuesta metodológica del estudiante no tiene sentido.
        Primero, el test de ratios de verosimilitud (TRV) permite
        comparar modelos, no parámetros. Segundo, aun cuando
        quisiéramos comparar los modelos, el TRV requiere que un
        modelo esté anidado en el otro, lo que no ocurre en este caso
        (son tres modelos independientes estimados en submuestras
        distintas).

    b.  Lo más sencillo es incluir los años como covariables en la
        tasa de envío:

        $$
        \mu_i(\mathbf{x}_i) = \gamma_0 + \gamma_1\,\text{Año}_{2023}
          + \gamma_2\,\text{Año}_{2024} + \gamma_3\,\text{Año}_{2025}.
        $$

        Con esto, podemos aplicar un test $F$ para evaluar si
        $\gamma_1 = \gamma_2$ y $\gamma_2 = \gamma_3$.

4.  De acuerdo al AIC, el mejor modelo es el NBD homogéneo (AIC =
    75.760). Recordando que la distribución posterior de la tasa
    individual en un modelo Poisson-Gamma es
    ...
    $$
    E(\lambda \mid n = 2) = \frac{r + n}{\alpha + 1}
      = \frac{3{,}52 + 2}{0{,}45 + 1} \approx 3{,}81
      \;\text{[respuestas/año]}.
    $$

5.  Hay múltiples opciones válidas. Algunos ejemplos:

    -   *Carrera:* es posible que algunas carreras respondan más que
        otras, ya sea por factores culturales o de carga académica. La
        dirección del signo dependerá de la carrera específica.
    -   *Tipo de encuesta:* es posible que algunos tipos de encuestas se
        contesten más que otros. Se puede postular que las encuestas de
        bienestar se contestan más que las de docencia, porque a los
        estudiantes les importan más los cambios en infraestructura.
    -   *Duración promedio:* si un estudiante recibe encuestas que duran
        más días, puede tener una mayor probabilidad de contestar y por
        tanto el efecto esperado sería positivo (aunque previsiblemente
        pequeño).
    -   *Número de encuestas recibidas (y su cuadrado):* si un
        estudiante recibe más encuestas, tiene un universo mayor para
        responder y por tanto el efecto podría ser positivo. Si recibe
        demasiadas encuestas, quizás deja de contestar, lo que se
        capturaría por un efecto cuadrático negativo.

    *Nota: las direcciones de los efectos son especulativas. Mientras se
    argumente adecuadamente, se considera correcta la respuesta.*

6.  Asumimos que el número de respuestas, condicionado en el número de
    encuestas recibidas $m_i$, se distribuye binomial con tasa
    $\theta_i(\mathbf{x}_i)$. La log-verosimilitud viene dada por:

    $$
    LL = \sum_i \ln\!\Big[\Pr(N_i = n_i \mid m_i)\Big]
       = \sum_i \ln\!\left[
         \binom{m_i}{n_i}\,
         \theta_i(\mathbf{x}_i)^{n_i}\,
         \big(1 - \theta_i(\mathbf{x}_i)\big)^{m_i - n_i}
         \right],
    $$
...
1.  We assume that the number of responses follows a Poisson
    distribution with rate $\lambda_i(\mathbf{x}_i)$. Since all
    students are active during the three years of observation, the
    exposure is $T = 3$. The log-likelihood is then given by:
    ...
    where $n_i$ is the number of responses from student $i$, obtained
    by counting the rows in the panel where $\text{NMat} = i$ and
    $\text{Response} = \text{Yes}$.

2.  The question is conceptually identical to the previous one, but
    estimated with different data. Assuming that the number of surveys
    received follows a Poisson distribution with rate
    $\mu_i(\mathbf{x}_i)$, the log-likelihood is given by:
    ...
    where $m_i$ is the number of surveys received by student $i$,
    obtained by counting the rows in the panel where
    $\text{NMat} = i$ (regardless of the response).

3.  The two parts can be answered separately:

    a.  The student's methodological proposal does not make sense.
        First, the likelihood ratio test (LRT) is used to compare
        models, not parameters. Second, even if we wanted to compare
        the models, the LRT requires that one model be nested within
        another, which is not the case here (they are three independent
        models estimated on different subsamples).

    b.  The simplest approach is to include year indicators as
        covariates in the sending rate:

        $$
        \mu_i(\mathbf{x}_i) = \gamma_0 + \gamma_1\,\text{Year}_{2023}
          + \gamma_2\,\text{Year}_{2024} + \gamma_3\,\text{Year}_{2025}.
        $$

        With this specification, we can apply an $F$ test to evaluate
        whether $\gamma_1 = \gamma_2$ and $\gamma_2 = \gamma_3$.

4.  According to the AIC, the best model is the homogeneous NBD (AIC =
    75,760). Recalling that the posterior distribution of the individual
    rate in a Poisson-Gamma model is

    $$
    \lambda \mid n \sim \text{Gamma}(\alpha + 1,\; r + n),
    $$

    we can compute the conditional expectation directly:
...
5.  There are many valid options. Some examples:

    -   *Field of study:* some fields may respond more than others, due
        to cultural factors or academic workload. The sign direction
        will depend on the specific field.
    -   *Survey type:* some survey types may elicit more responses than
        others. One could hypothesize that welfare surveys are answered
        more than teaching surveys, because students care more about
        infrastructure changes.
    -   *Average duration:* if a student receives surveys that stay open
        longer, they may have a higher probability of responding, so the
        expected effect would be positive (though likely small).
    -   *Number of surveys received (and its square):* if a student
        receives more surveys, they have a larger universe in which to
        respond, so the effect could be positive. If they receive too
        many surveys, they may stop responding, which would be captured
        by a negative quadratic effect.

    *Note: the effect directions are speculative. As long as the
    argument is well-reasoned, the answer is considered correct.*

6.  We assume that the number of responses, conditional on the number of
    surveys received $m_i$, follows a binomial distribution with rate
    $\theta_i(\mathbf{x}_i)$. The log-likelihood is given by:
    ...
    where $m_i$ and $n_i$ are defined as in parts 1 and 2.
