...
### Checklist de modelamiento {data-en="Modeling Checklist"}
...
de los datos antes de aplicar cualquier modelo. La siguiente lista
organiza las decisiones clave:
...
before applying any model. The following list organizes the key
decisions:
...
restricciones al nivel de agregación mínimo. Agregar los datos a un
nivel de agregación más alto puede ser más fácil, ya que se requiere
menos detalle y se puede tener una visión más general. Sin embargo,
puede llevar a una pérdida de precisión por underfitting. Por otro lado,
si la cantidad de datos es limitada, reducir el nivel de agregación
puede significar overfitting.
...
constraints on the minimum aggregation level. Aggregating data to a
higher level can be easier, as less detail is required and a more
general view can be obtained. However, it may lead to a loss of
precision due to underfitting. On the other hand, if the amount of
data is limited, reducing the aggregation level may result in
overfitting.
...
-   *Descomposición por índices:* Dividir el conjunto de datos en
    subconjuntos según algún criterio y ajustar modelos de regresión
    separados para cada subconjunto.

-   *Descomposición por componentes latentes:* La variable dependiente
    puede descomponerse de manera natural en componentes latentes. Un
    ejemplo típico es la descomposición de las ventas en el número de
    compras y el número de unidades por compra.

-   *Caso del cero inflado (zero inflated):* Se distingue entre dos
    procesos distintos: la incidencia de compra (probabilidad de que
    ocurra) y el monto de compra (valor condicional a que se haya
    realizado).
...
-   *Decomposition by indices:* Divide the dataset into subsets
    according to some criterion and fit separate regression models
    for each subset.

-   *Decomposition by latent components:* The dependent variable can
    be naturally decomposed into latent components. A typical example
    is the decomposition of sales into the number of purchases and
    the number of units per purchase.

-   *Zero-inflated case:* A distinction is made between two different
    processes: purchase incidence (probability that it occurs) and
    purchase amount (value conditional on the purchase having taken
    place).
...
interpretación y el ajuste del modelo. Un ejemplo común es el modelo
doble log, en el cual se aplican logaritmos a las variables dependientes
e independientes para transformar la relación no lineal en una relación
lineal.
...
interpretation and fit. A common example is the double-log model, in
...
-   *Forward*: las variables se agregan una por una, comenzando con la
    que proporciona el mejor ajuste.
-   *Backward*: todas las variables se incluyen inicialmente y se
    eliminan una por una.

Otro enfoque es la penalización de uso de parámetros no nulos:
...
-   *Forward*: variables are added one by one, starting with the one
    that provides the best fit.
-   *Backward*: all variables are initially included and removed one
    by one.

Another approach is the penalization of nonzero parameters:
...
Aunque los enfoques automáticos son convenientes, no exploran todas las
posibles combinaciones de variables y pueden generar conjuntos poco
intuitivos. Una alternativa es utilizar una mixtura entre métodos
automáticos y manuales, eligiendo aquellas variables que mejoran el
modelo en ajuste y que igualmente tienen un sentido interpretativo.

:::

::: {.lang-en}

Although automatic approaches are convenient, they do not explore all
possible variable combinations and may produce unintuitive sets. An
alternative is to use a mixture of automatic and manual methods,
choosing those variables that improve model fit while also having
...
el rendimiento de los modelos. Es recomendable considerar la inclusión
de un índice si se cumplen las siguientes condiciones:

a)  *Relevancia para el problema de gestión*: el índice debe
    proporcionar una distinción significativa entre diferentes aspectos
    del problema.
...
c)  *Suficiencia de datos*: deben estar disponibles suficientes datos
    para estimar de manera confiable los parámetros desagregados.

d)  *Índices y variables binarias*: los índices pueden representarse
    mediante variables dummy. La notación de índices suele ser más
    compacta y se prefiere en la mayoría de los casos.
...
can significantly influence model interpretation and performance. It
is advisable to consider including an index if the following conditions
are met:

a)  *Relevance to the management problem*: the index must provide a
    meaningful distinction between different aspects of the problem.

b)  *Differences in the behavior of the dependent variable*: the
    dependent variable must exhibit distinct behaviors for each index.

c)  *Data sufficiency*: enough data must be available to reliably
    estimate the disaggregated parameters.

d)  *Indices and binary variables*: indices can be represented using
    dummy variables. Index notation is usually more compact and is
...
Los modelos lineales generalizados mantienen la linealidad en los
parámetros, pero la respuesta del modelo puede tener una forma funcional
distinta. Dentro de esta familia se encuentran:

-   Regresión de Poisson (variable dependiente positiva y discreta).
-   Regresión logística (se predice el número de sucesos de un total de
    intentos).
...
Generalized linear models maintain linearity in the parameters, but the
model response can have a different functional form. Within this family
we find:

-   Poisson regression (positive and discrete dependent variable).
-   Logistic regression (predicts the number of successes out of a total
    number of trials).
...
### ¿Qué es Machine Learning? {data-en="What Is Machine Learning?"}

::: {.lang-es}

El aprendizaje automático es una rama de la inteligencia artificial que
se centra en el desarrollo de algoritmos y modelos que permiten a las
computadoras aprender patrones y realizar tareas específicas sin ser
programadas explícitamente [@james2021; @hastie2009]. En lugar de seguir
instrucciones detalladas, los sistemas de aprendizaje automático
utilizan datos para mejorar su rendimiento en una tarea particular a lo
largo del tiempo.
...
the development of algorithms and models that allow computers to learn
patterns and perform specific tasks without being explicitly programmed
[@james2021; @hastie2009]. Instead of following detailed instructions,
machine learning systems use data to improve their performance on a
particular task over time.

:::

### ¿La regresión cuenta como un modelo de ML? {data-en="Is Regression a Machine Learning Model?"}
...
El hecho de tener que seleccionar variables, aplicar transformaciones
y verificar supuestos estadísticos la hace menos automática. Un
acercamiento más próximo a ML se da al utilizar técnicas de selección
automática como Ridge o LASSO. En este curso se entenderá como ML a
...
Although regression uses data to learn from them, it is not usually
considered a fully-fledged machine learning model. The need to select
variables, apply transformations, and verify statistical assumptions
makes it less automatic. A closer approximation to ML arises when using
automatic selection techniques such as Ridge or LASSO. In this course,
ML will be understood as any model that can automatically learn about
the set of predictive variables and their functional forms.
...
Este procedimiento se aplica recursivamente hasta alcanzar un criterio de parada (profundidad máxima, número mínimo de observaciones en un nodo, o reducción de costo insuficiente). Para evitar el sobreajuste, se emplea la **poda** (*pruning*): se construye un árbol grande y luego se eliminan ramas que contribuyen marginalmente, usando el criterio de costo-complejidad $R_\alpha(T) = R(T) + \alpha|T|$, donde $|T|$ es el número de hojas y $\alpha$ se calibra por validación cruzada [@tobar2024].

El resultado es un modelo altamente interpretable, donde cada nodo tiene una regla de decisión explícita, y que detecta automáticamente interacciones y no linealidades. Además, los árboles manejan variables numéricas y categóricas de forma nativa. No obstante, su principal limitación es la alta varianza: pequeños cambios en los datos pueden alterar significativamente la estructura del árbol. La búsqueda voraz tampoco garantiza encontrar el árbol óptimo global (problema NP-completo), y sin poda o restricciones de profundidad tienden al sobreajuste.

:::

::: {.lang-en}

This procedure is applied recursively until a stopping criterion is met (maximum depth, minimum number of observations in a node, or insufficient cost reduction). To avoid overfitting, **pruning** is used: a large tree is built and then branches that contribute marginally are removed, using the cost-complexity criterion $R_\alpha(T) = R(T) + \alpha|T|$, where $|T|$ is the number of leaves and $\alpha$ is calibrated by cross-validation [@tobar2024].

The result is a highly interpretable model where each node has an explicit decision rule, and which automatically detects interactions and non-linearities. Trees also handle numeric and categorical variables natively. However, their main limitation is high variance: small changes in the data can significantly alter the tree structure. The greedy search does not guarantee finding the globally optimal tree (an NP-complete problem), and without pruning or depth constraints, trees tend to overfit.
...
## Sumario de la unidad {data-en="Unit Summary"}
